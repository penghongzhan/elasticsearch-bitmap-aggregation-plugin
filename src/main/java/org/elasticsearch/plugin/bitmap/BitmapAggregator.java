...
public class BitmapAggregator extends NumericMetricsAggregator.SingleValue {
        ...
        return new LeafBucketCollectorBase(sub, values) {
            ...
            public void collect(int doc, long bucket) throws IOException {
                ...
                for (int i = 0; i < valuesCount; i++) {
                    BytesRef valueAt = values.valueAt(i);
    ...
    public InternalAggregation buildAggregation(long bucket) {
        ...
        return new InternalBitmap(name, sums.get(bucket), format, pipelineAggregators(), metaData());
    }

    @Override
    public InternalAggregation buildEmptyAggregation() {
        LOG.debug("BitmapAggregator buildEmptyAggregation");
        return new InternalBitmap(name, new RoaringBitmap(), format, pipelineAggregators(), metaData());
    }
